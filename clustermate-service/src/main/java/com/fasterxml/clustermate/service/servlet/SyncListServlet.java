    ...
    extends ServletBase
{
    ...
    {
        String str = request.getQueryParameter(HTTPConstants.HTTP_QUERY_PARAM_SINCE);
        if (str == null) {
            response = _syncHandler.missingArgument(response, HTTPConstants.HTTP_QUERY_PARAM_SINCE);
        } else {
            long since = -1;
            try {
            ...
            if (since < 0L) {
                response = _syncHandler.invalidArgument(response, HTTPConstants.HTTP_QUERY_PARAM_SINCE, str);
            } else {
